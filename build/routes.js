...
const mcp_client_js_1 = require("./mcp/client/mcp-client.js");
...
mainRouter.get('/', async (_req, res) => {
    try {
        res.json(await nseIndia.getDataByEndpoint(index_1.ApiList.MARKET_STATUS));
    }
    catch (error) {
...
mainRouter.get('/api/glossary', async (_req, res) => {
    try {
        res.json(await nseIndia.getDataByEndpoint(index_1.ApiList.GLOSSARY));
    }
    catch (error) {
        res.status(400).json(error);
    }
...
mainRouter.get('/api/marketTurnover', async (_req, res) => {
    try {
        res.json(await nseIndia.getDataByEndpoint(index_1.ApiList.MARKET_TURNOVER));
    }
    catch (error) {
        res.status(400).json(error);
    }
...
mainRouter.get('/api/equityMaster', async (_req, res) => {
    try {
        res.json(await nseIndia.getDataByEndpoint(index_1.ApiList.EQUITY_MASTER));
    }
    catch (error) {
        res.status(400).json(error);
    }
...
mainRouter.get('/api/holidays', async (req, res) => {
    try {
        ...
        if (type === 'clearing') {
            res.json(await nseIndia.getDataByEndpoint(index_1.ApiList.HOLIDAY_CLEARING));
        }
        else {
            res.json(await nseIndia.getDataByEndpoint(index_1.ApiList.HOLIDAY_TRADING));
        }
    }
    catch (error) {
        res.status(400).json(error);
...
mainRouter.get('/api/circulars', async (req, res) => {
    try {
        ...
        if (isLatest === 'true') {
            res.json(await nseIndia.getDataByEndpoint(index_1.ApiList.LATEST_CIRCULARS));
        }
        else {
            res.json(await nseIndia.getDataByEndpoint(index_1.ApiList.CIRCULARS));
        }
    }
    catch (error) {
        res.status(400).json(error);
...
mainRouter.get('/api/mergedDailyReports', async (req, res) => {
    try {
        const { key } = req.query;
        if (key === 'debt') {
            res.json(await nseIndia.getDataByEndpoint(index_1.ApiList.MERGED_DAILY_REPORTS_DEBT));
        }
        else if (key === 'derivatives') {
            res.json(await nseIndia.getDataByEndpoint(index_1.ApiList.MERGED_DAILY_REPORTS_DERIVATIVES));
        }
        else {
            res.json(await nseIndia.getDataByEndpoint(index_1.ApiList.MERGED_DAILY_REPORTS_CAPITAL));
        }
    }
    catch (error) {
        res.status(400).json(error);
...
mainRouter.get('/api/allIndices', async (_req, res) => {
    try {
        const allIndices = await nseIndia.getDataByEndpoint(index_1.ApiList.ALL_INDICES);
        res.json(allIndices);
    }
    catch (error) {
        res.status(400).json(error);
...
mainRouter.get('/api/indexNames', async (_req, res) => {
    try {
        const indexNames = await nseIndia.getDataByEndpoint(index_1.ApiList.INDEX_NAMES);
        res.json(indexNames);
    }
    catch (error) {
        res.status(400).json(error);
...
mainRouter.get('/api/equity/technicalIndicators/:symbol', async (req, res) => {
    try {
        const { symbol } = req.params;
        const { period, smaPeriods, emaPeriods, rsiPeriod, bbPeriod, bbStdDev, showOnlyLatest } = req.query;
        // Parse query parameters
        const options = {};
        if (period) {
            options.period = parseInt(period);
        }
        if (smaPeriods) {
            options.smaPeriods = smaPeriods.split(',').map(p => parseInt(p.trim()));
        }
        if (emaPeriods) {
            options.emaPeriods = emaPeriods.split(',').map(p => parseInt(p.trim()));
        }
        if (rsiPeriod) {
            options.rsiPeriod = parseInt(rsiPeriod);
        }
        if (bbPeriod) {
            options.bbPeriod = parseInt(bbPeriod);
        }
        if (bbStdDev) {
            options.bbStdDev = parseFloat(bbStdDev);
        }
        const indicators = await nseIndia.getTechnicalIndicators(symbol, options.period || 200, options);
        ...
        const roundArrayTo2Decimals = (arr) => {
            return arr.map(value => roundTo2Decimals(value));
        };
        if (showLatest) {
            ...
            latestIndicators.rsi = roundTo2Decimals(indicators.rsi.length > 0 ? indicators.rsi[indicators.rsi.length - 1] : null);
            latestIndicators.macd = {
                macd: roundTo2Decimals(indicators.macd.macd.length > 0 ? indicators.macd.macd[indicators.macd.macd.length - 1] : null),
                signal: roundTo2Decimals(indicators.macd.signal.length > 0 ? indicators.macd.signal[indicators.macd.signal.length - 1] : null),
            ...
            latestIndicators.williamsR = roundTo2Decimals(indicators.williamsR.length > 0 ? indicators.williamsR[indicators.williamsR.length - 1] : null);
            latestIndicators.atr = roundTo2Decimals(indicators.atr.length > 0 ? indicators.atr[indicators.atr.length - 1] : null);
            latestIndicators.adx = roundTo2Decimals(indicators.adx.length > 0 ? indicators.adx[indicators.adx.length - 1] : null);
            latestIndicators.obv = roundTo2Decimals(indicators.obv.length > 0 ? indicators.obv[indicators.obv.length - 1] : null);
            latestIndicators.cci = roundTo2Decimals(indicators.cci.length > 0 ? indicators.cci[indicators.cci.length - 1] : null);
            latestIndicators.mfi = roundTo2Decimals(indicators.mfi.length > 0 ? indicators.mfi[indicators.mfi.length - 1] : null);
            latestIndicators.roc = roundTo2Decimals(indicators.roc.length > 0 ? indicators.roc[indicators.roc.length - 1] : null);
            latestIndicators.momentum = roundTo2Decimals(indicators.momentum.length > 0 ? indicators.momentum[indicators.momentum.length - 1] : null);
            latestIndicators.ad = roundTo2Decimals(indicators.ad.length > 0 ? indicators.ad[indicators.ad.length - 1] : null);
            latestIndicators.vwap = roundTo2Decimals(indicators.vwap.length > 0 ? indicators.vwap[indicators.vwap.length - 1] : null);
...
mainRouter.get('/api/gainersAndLosers/:indexSymbol', async (req, res) => {
    try {
        res.json(await (0, helpers_1.getGainersAndLosersByIndex)(req.params.indexSymbol));
    }
    catch (error) {
        res.status(400).json(error);
    }
...
mainRouter.get('/api/mostActive/:indexSymbol', async (req, res) => {
    try {
        res.json(await (0, helpers_1.getMostActiveEquities)(req.params.indexSymbol));
    }
    catch (error) {
        res.status(400).json(error);
    }
...
mainRouter.post('/api/mcp/query', async (req, res) => {
    try {
        const { query, sessionId: providedSessionId, userId, model, temperature, max_tokens, includeContext, updatePreferences, useMemory, maxIterations, enableDebugLogging } = req.body;
        ...
        const originalDebugState = mcp_client_js_1.mcpClient.isDebugLoggingEnabled();
        // Temporarily enable debug logging if requested
        if (shouldEnableDebug) {
            mcp_client_js_1.mcpClient.setDebugLogging(true);
        }
        try {
            const result = await mcp_client_js_1.mcpClient.processQuery({
                query,
                sessionId,
                userId,
                model,
            ...
            mcp_client_js_1.mcpClient.setDebugLogging(originalDebugState);
        }
    }
    catch (error) {
...
mainRouter.get('/api/mcp/tools', async (_req, res) => {
    try {
        const tools = mcp_client_js_1.mcpClient.getAvailableTools();
        res.json({ tools });
    }
    catch (error) {
...
mainRouter.get('/api/mcp/test', async (_req, res) => {
    try {
        ...
        const isConnected = await mcp_client_js_1.mcpClient.testConnection();
        if (isConnected) {
            res.json({
                status: 'success',
...
mainRouter.get('/api/mcp/functions', async (_req, res) => {
    try {
        const functions = mcp_client_js_1.mcpClient.getOpenAIFunctions();
        res.json({ functions });
    }
    catch (error) {
...
mainRouter.get('/api/mcp/session/:sessionId', async (req, res) => {
    try {
        ...
        const sessionInfo = mcp_client_js_1.mcpClient.getSessionInfo(sessionId);
        if (!sessionInfo) {
...
mainRouter.get('/api/mcp/session/:sessionId/history', async (req, res) => {
    try {
        ...
        const history = mcp_client_js_1.mcpClient.getConversationHistory(sessionId, maxMessages ? parseInt(maxMessages) : undefined);
        if (history.length === 0) {
            return res.status(404).json({
...
mainRouter.put('/api/mcp/session/:sessionId/preferences', async (req, res) => {
    try {
        ...
        mcp_client_js_1.mcpClient.updateUserPreferences(sessionId, preferences);
        res.json({
...
mainRouter.delete('/api/mcp/session/:sessionId/clear', async (req, res) => {
    try {
        const { sessionId } = req.params;
        mcp_client_js_1.mcpClient.clearSession(sessionId);
        res.json({
...
mainRouter.get('/api/mcp/session/:sessionId/export', async (req, res) => {
    try {
        const { sessionId } = req.params;
        const sessionData = mcp_client_js_1.mcpClient.exportSessionData(sessionId);
        if (!sessionData) {
            return res.status(404).json({
                error: 'Session not found'
            });
...
mainRouter.post('/api/mcp/cleanup', async (_req, res) => {
    try {
        mcp_client_js_1.mcpClient.cleanupExpiredSessions();
        res.json({
...
mainRouter.get('/api/mcp/session/:sessionId/context-stats', async (req, res) => {
    try {
        const { sessionId } = req.params;
        const stats = await mcp_client_js_1.mcpClient.getContextStats(sessionId);
        res.json(stats);
    }
    catch (error) {
...
mainRouter.post('/api/mcp/session/:sessionId/summarize', async (req, res) => {
    try {
        const { sessionId } = req.params;
        const summary = await mcp_client_js_1.mcpClient.forceContextSummarization(sessionId);
        if (!summary) {
            return res.status(404).json({
                error: 'Session not found'
            });
...
mainRouter.get('/api/mcp/session/:sessionId/context-window', async (req, res) => {
    try {
        const config = mcp_client_js_1.mcpClient.getContextWindowConfig();
        res.json(config);
    }
    catch (error) {
...
mainRouter.put('/api/mcp/session/:sessionId/context-window', async (req, res) => {
    try {
        ...
        mcp_client_js_1.mcpClient.updateContextWindowConfig(config);
        res.json({
            message: 'Context window configuration updated successfully',
            config: mcp_client_js_1.mcpClient.getContextWindowConfig()
        });
    }
    catch (error) {
...
mainRouter.get('/api/mcp/session/:sessionId/summarization/last', async (req, res) => {
    try {
        const { sessionId } = req.params;
        if (!mcp_client_js_1.mcpClient.isMemoryEnabled()) {
            return res.status(500).json({ error: 'Memory manager not enabled' });
        }
        const lastSummarization = mcp_client_js_1.mcpClient.getLastSummarization(sessionId);
...
mainRouter.get('/api/mcp/session/:sessionId/summarization/history', async (req, res) => {
    try {
        ...
        const history = mcp_client_js_1.mcpClient.getSummarizationHistory(sessionId, limit);
        res.json({
            sessionId,
...
mainRouter.get('/api/mcp/session/:sessionId/summarization/summary', async (req, res) => {
    try {
        ...
        const summary = mcp_client_js_1.mcpClient.getSummarizationSummary(sessionId);
        if (!summary) {
            return res.status(404).json({
...
mainRouter.get('/api/mcp/session/:sessionId/openai-messages', async (req, res) => {
    try {
        ...
        const data = mcp_client_js_1.mcpClient.getOpenAIMessages(sessionId);
        if (!data) {
        ...
        const openaiMessages = [
            ...
            ...data.conversationHistory.map((msg) => {
                ...
                return ({
                    ...
                    metadata: {
                        ...msg.metadata,
                        is_summary: ((_a = msg.metadata) === null || _a === void 0 ? void 0 : _a.isSummary) || false,
        ...
        const summaryMessages = openaiMessages.filter(m => m.metadata && 'is_summary' in m.metadata && m.metadata.is_summary);
        const stats = {
